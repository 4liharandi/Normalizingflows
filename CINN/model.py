...
from FrEIA.modules import GLOWCouplingBlock as AffineCouplingLayer

scaleop = transforms.Compose([transforms.Resize(64, interpolation=transforms.InterpolationMode.NEAREST)])
...
class CINN(nn.Module):
    ...
    def build_inn(self):
        ...
        '''
        AffineCouplingLayer according to design from paper. Note, this is only the coupling
        part itself, and does not include ActNorm, invertible 1x1 convolutions, etc. Renamed in order
        to avoid confusion, since original name GLOWCouplingBlock is misleading since it only contains
        the affine coupling layer.
        '''
        for k in range(c.cluster_1):
            nodes.append(Ff.Node(nodes[-1], AffineCouplingLayer,
        ...
        nodes.append(Ff.Node([s.out0 for s in split_nodes] + [nodes[-1].out0],
                             Fm.Concat, {'dim':0}, name=f"Concat1d"))
    ...
    def forward(self, target, image):
        if image.size[2] != 64:
            image = scaleop(target)
        z,jac_det = self.cinn(target, c=self.cond_net(image))
        return z, jac_det
