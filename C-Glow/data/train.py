...
def sample_from(num_show, num_samples, mean_samples=50):
    ...
    with torch.no_grad():
        ...
        for j in range(mean_samples):
            z = torch.randn(c.batch_size, 768, 4, 4).cuda()
...
def train():
    ...
    try:
        for epoch in range(saved_epoch + 1, int(c.n_epochs) + 1):
            ...
            for i_batch, data_tuple in tqdm(enumerate(data_iter),
                ...
                z, log_j = cinn(target, image)
                nll = torch.mean(z ** 2) / 2 - torch.mean(log_j) / c.input_dim
                nll.backward()
